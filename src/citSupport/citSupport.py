...
def initVariables():
    ...
    for v, f in initVariableFUNC.items():
        newValue = f()
...
def updateVariables():
    variables = {}
    # Evaluate all variables so they stay constant for this set of commands.
    for v, f in testVariableFUNC.items():
        newValue = f()
        # If a function returns None, it's out of values.
        # Pass this uplevel. We'll probably want to stop.
        if newValue is None:
...
def locate(test, variables):
    ...
    try:
        ...
        if e.errno != errno.EEXIST:
            print("os.mkdir(%s) returns %d: %s" % (e.filename, e.errno, e.strerror), file=sys.stderr)
            sys.exit(1)
    ...
    while (result == 0 or continueOnError) and not doExit:
        ...
        if result != 0:
            ...
            for k, v in variables.items():
                print('%s: %s "%s"' % (modName, k, v), file=sys.stderr)
